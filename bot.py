...
# ✅ Activos con nombre EXACTO que sí existen en IQ Option
ACTIVOS = [
    "EURUSD-OTC",
    "GBPUSD-OTC",
    "EURJPY-OTC",
    ...
    for intento in range(REINTENTOS):
        try:
            ...
            if estado and id_op > 0:
                ...
                logger.info(f"✅ {nombre} | ID: {id_op} | Saldo: ${saldo_final}")
                exito = True
                id_operacion = id_op
                break
            time.sleep(ESPERA)
...
def bucle_principal():
    ...
    logger.info("="*70)
    logger.info("🤖 BOT | 15 operaciones por cuenta | Control Telegram")
    logger.info("="*70)
    ...
    while BOT_ACTIVO:
        try:
            if OPERACIONES_C1 >= MAX_OPER_C1 and OPERACIONES_C2 >= MAX_OPER_C2:
                logger.info("✅ ¡Completadas las 30 operaciones! Bot detenido.")
                BOT_ACTIVO = False
                break
            ...
            if senal_actual and SEG_INICIO <= segundos <= SEG_FIN:
                ...
                if res1["ok"]:
                    logger.info(f"✅ C1: {OPERACIONES_C1}/15")
                if res2["ok"]:
                    logger.info(f"✅ C2: {OPERACIONES_C2}/15")
...
async def escuchar_comandos():
    ...
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        logger.warning("⚠️ Sin credenciales de Telegram, bot funcionará sin control remoto")
        return

    bot = Bot(token=TELEGRAM_TOKEN)
    logger.info("📡 Escuchando comandos Telegram...")

    while True:
        try:
            actualizaciones = await bot.get_updates(timeout=10)
            for upd in actualizaciones:
                if not upd.message or str(upd.message.chat_id) != TELEGRAM_CHAT_ID:
                    continue
                ...
                if texto == "/start":
                    if not BOT_ACTIVO:
                        ...
                        await bot.send_message(TELEGRAM_CHAT_ID, "✅ Bot iniciado. 15 operaciones por cuenta.")
                    else:
                        await bot.send_message(TELEGRAM_CHAT_ID, "ℹ️ Ya está activo.")

                elif texto == "/stop":
                    BOT_ACTIVO = False
                    await bot.send_message(TELEGRAM_CHAT_ID, "⏹️ Bot detenido.")
