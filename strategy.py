...
# ✅ NOMBRE Y PARÁMETROS COINCIDENTES CON bot.py
# ==================================================

def get_reversal_signal(df, tolerancia_nivel=0.0028, ventana_niveles=5):
    # Requiere menos velas para operar más frecuentemente
    if len(df) < 10:
        return None
    ...
    if cond_compra:
        senal = "call"
        tipo = "soporte"   # ← Coincide con mensajes de tu bot
        fuerza = 40
        if e13_1 > e21_1:
            fuerza += 5
        if c1 > c2:
    ...
    if cond_venta:
        senal = "put"
        tipo = "resistencia"  # ← Coincide con mensajes de tu bot
        fuerza = 40
        if e13_1 < e21_1:
            fuerza += 5
        if c1 < c2:
